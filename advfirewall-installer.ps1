...
if (-not $Administrator) {
    ...
    if ($args.Length -gt 1) {
        ...
        if ($args[1].Contains("notification")) {
            $Elevate = $true
        }
        ...
        if ($args[0].Contains("notification")) {
            $Elevate = $true
        }
    }
    if ($Elevate) {
...
if ($args.Length -gt 0) {
    if ($args[0].Contains("local")) {
        ...
        [string] $TaskName = "advfirewall-notification"
        [string] $TaskScript = (Join-Path -Path $PSScriptRoot -ChildPath "advfirewall-notification.ps1")
        [string] $TaskDescription = $Messages."Windows Firewall Notification"
        [string] $TaskCommand = (Join-Path -Path $PSHOME -ChildPath "powershell.exe")
        [string] $TaskArguments = "-NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$TaskScript`""
...
'@
        if (Get-Command -Name Get-ScheduledTask -ErrorAction SilentlyContinue) {
            if (Get-ScheduledTask -TaskName $TaskName -TaskPath "\" -ErrorAction SilentlyContinue) {
                ...
                Register-ScheduledTask -Action $Action -Trigger $Trigger -TaskName $TaskName -Description $TaskDescription -TaskPath "\" -Settings $Settings
        ...
        if (-not (Test-Path -Path "HKCU:\Software\Classes\advfirewall\shell\open\command" -ErrorAction SilentlyContinue)) {
            ...
            Set-ItemProperty -Path "HKCU:\Software\Classes\advfirewall\shell\open\command" -Name "(Default)" `
                             -Value "$(Join-Path -Path $PSHOME -ChildPath "powershell.exe") -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$PSScriptRoot\advfirewall-notification-helper.ps1`" `"%1`"" `
                             -Verbose
        }
...
'@
        ...
        if ($args.Length -gt 1) {
            if ($args[1].Contains("local")) {
                ...
                Remove-ShortCut -Link (Join-Path -Path $Path -ChildPath ("{0}.lnk" -f $Messages."Windows Firewall Notification"))
                Add-Type -AssemblyName System.Windows.Forms
